...
export default async function AdminInvoicePrintPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  const invoice = await db.billingInvoice.findUnique({
    where: { id },
    include: { restaurant: true, paymentAccount: true }
  });
  if (!invoice) notFound();

  const fallbackAccounts = invoice.paymentAccount
    ? []
    : await db.billingPaymentAccount.findMany({
        where: { isActive: true, OR: [{ restaurantId: null }, { restaurantId: invoice.restaurantId }] },
  ...
  return (
    <main className="mx-auto max-w-3xl bg-white p-4 text-foreground print:p-0">
      <div className="mb-4 flex gap-2 print:hidden">
        ...
        <Link href="/admin/billing"><Button variant="outline">Back to Billing</Button></Link>
      </div>
      <InvoiceDocument invoice={invoice} accounts={invoice.paymentAccount ? [invoice.paymentAccount] : fallbackAccounts} />
    </main>
  );
}

function InvoiceDocument({ invoice, accounts }: { invoice: any; accounts: any[] }) {
  return (
    <section className="rounded-lg border p-8 print:border-0">
      ...
      <div className="grid gap-6 border-b py-5 sm:grid-cols-2">
        <div>
          ...
          <p className="mt-2">{invoice.restaurant.name}</p>
          <p>{invoice.restaurant.branchName}</p>
          <p>{invoice.restaurant.city}</p>
      ...
      <div className="grid gap-4 border-t pt-5 sm:grid-cols-2">
        ...
        <div>
          ...
          {invoice.paymentClaimedAt ? <p className="mt-3 text-sm">Manager payment request submitted: {formatPkDateTime(invoice.paymentClaimedAt)}</p> : null}
